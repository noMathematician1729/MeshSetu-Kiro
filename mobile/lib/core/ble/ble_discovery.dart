...
///   [DiscoveryMetadata] rides as manufacturer data instead, tagged with the
///   Bluetooth SIG's reserved "for testing" company identifier (`0xFFFF`) —
...
abstract final class MeshAdvertiser {
  static const int testCompanyId = 0xFFFF;
  ...
  static Future<void> start(DiscoveryMetadata metadata) =>
      UniversalBlePeripheral.startAdvertising(
        ...
        localName: 'MeshSetu',
        manufacturerData: ManufacturerData(testCompanyId, metadata.encode()),
      );
  ...
  static Future<List<DiscoveredPeer>> scan({
    ...
    final subscription = UniversalBle.scanStream.listen((device) {
      for (final data in device.manufacturerDataList) {
        if (data.companyId != MeshAdvertiser.testCompanyId) continue;
