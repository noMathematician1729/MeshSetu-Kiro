...
export function demoSiteKey(siteId) {
...
function u64(value) { return BigInt(value?.toString?.() ?? value ?? 0).toString(); }
export async function decryptPacket(packet, expectedObjectId, expectedSiteId) {
    if (packet.length <= 1 + 2 + IV_BYTES + TAG_BYTES)
        throw new Error('encrypted object is too short');
    let offset = 0;
    if (packet.readUInt8(offset++) !== VERSION)
        throw new Error('unsupported crypto format');
    const siteLength = packet.readUInt16BE(offset);
    offset += 2;
    if (siteLength > packet.length - offset - IV_BYTES - TAG_BYTES)
        throw new Error('invalid site metadata');
    const site = packet.subarray(offset, offset + siteLength);
    offset += siteLength;
    const iv = packet.subarray(offset, offset + IV_BYTES);
    offset += IV_BYTES;
    ...
    const decoded = type.toObject(type.decode(plaintext), { longs: String, enums: Number, bytes: Buffer, defaults: true });
    if (u64(decoded.objectId) !== expectedObjectId)
        throw new Error('object id mismatch');
    if (decoded.siteId !== siteId)
        throw new Error('envelope site id mismatch');
    ...
    let payload = null;
    if (envelope.payloadType === 'structuredSos')
        payload = sosSchema.parse(JSON.parse(envelope.payload.toString('utf8')));
    if (envelope.payloadType === 'voiceObject') {
        ...
        if (crypto.createHash('sha256').update(bytes).digest('hex') !== voice.sha256)
            throw new Error('voice integrity check failed');
        payload = { ...voice, bytes };
    }
