...
export class EventStore {
    ...
    async all() { if (!this.pool)
        return [...this.memory.values()]; const result = await this.pool.query('SELECT * FROM sos_incidents ORDER BY CASE priority WHEN $1 THEN 0 WHEN $2 THEN 1 WHEN $3 THEN 2 ELSE 3 END, received_at_ms DESC', ['p0Critical', 'p1High', 'p2Normal']); return result.rows; }
    async get(id) { if (!this.pool)
        return this.memory.get(id); const result = await this.pool.query('SELECT * FROM sos_incidents WHERE event_id=$1', [id]); return result.rows[0]; }
    async status(id, value) { const current = await this.get(id); if (!current)
        return undefined; const next = { ...current, status: value }; return this.upsert(next); }
}
